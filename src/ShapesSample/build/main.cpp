                   ...
                   PSTR cmdLine, int showCmd)
{
    ...
    LogInfo("Starting Shapes Sample");

    ShapesApp sample(hInstance);
...
ShapesApp::~ShapesApp()
{
	...
	SafeRelease(mWireframeRS);
}

bool ShapesApp::VInit()
{
	if(!D3dApp::VInit())
		return false;
...
void ShapesApp::VOnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void ShapesApp::VOnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void ShapesApp::VOnMouseMove(WPARAM btnState, int x, int y)
{
...
{
    CHRR(EffectHelper::LoadBinaryEffectFromFile("data/fx/default.fxo", md3dDevice, mFX));
